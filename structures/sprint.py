...
class Sprint:
    ...
    async def complete(self, context=None, bot=None):
        ...
        :return:
        """
        guild_id = self._guild if context is None else context.guild.id

        # Print the 'Results coming up shortly' message
        await self.say(lib.get_string('sprint:resultscomingsoon', guild_id), context, bot)
        ...
        for user_id in users:
            ...
            user = User(user_id, guild_id, context=context, bot=bot, channel=self.get_channel())
            ...
            if user_sprint['sprint_type'] == Sprint.SPRINT_TYPE_NO_WORDCOUNT:
                ...
                if user_sprint['ending_wc'] > 0 and user_sprint['ending_wc'] != user_sprint['starting_wc']:
                    ...
                    event = Event.get_by_guild(guild_id)
        ...
        if len(results) > 0:
            ...
            position = 1
            message = lib.get_string('sprint:results:header', guild_id)
            for result in results:

                if result['type'] == Sprint.SPRINT_TYPE_NO_WORDCOUNT:
                    message = message + lib.get_string('sprint:results:row:nowc', guild_id).format(result['user'].get_mention(), result['xp'])
                else:

                    message = message + lib.get_string('sprint:results:row', guild_id).format(position, result['user'].get_mention(), result['wordcount'], result['wpm'], result['xp'])

                    # If it's a new PB, append that string as well
                    if result['wpm_record'] is True:
                        message = message + lib.get_string('sprint:results:pb', guild_id)

                message = message + '\n'
                position += 1

        else:
            message = lib.get_string('sprint:nowordcounts', guild_id)
        ...
        # remove this sprint from the instance dict
        self.__class__._instances_lock.acquire()
        self.__class__._instances.pop(guild_id)
        self.__class__._instances_lock.release()
